...
Paddle::Paddle(int x, int y, int width, int height)
    : speed(2) // Set the speed of the paddle
{
    paddleRect.x = x;
    paddleRect.y = y;
    paddleRect.w = width;
...
void Paddle::handleInput(const Uint8* keystates, SDL_Scancode up, SDL_Scancode down) {
    // Move the paddle based on input
    if (keystates[up] && paddleRect.y > 0) {
        paddleRect.y -= speed; // Move up
    }
    if (keystates[down] && paddleRect.y < 600 - paddleRect.h) {
